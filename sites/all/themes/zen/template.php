...
/**
 * Retourne les éléments d'un menu : params : $menuName (ex : main-menu)
 */
function get_menu_links($menuName){
  $menu = menu_tree_all_data($menuName);
  $output = '';
  foreach ($menu as $key => $value) {
    $output .= '
      <div class="block_menu">
        <h2>'.l($value['link']['link_title'], $value['link']['link_path']).'</h2>';
        if (!empty($value['below'])) {
          $output .= '<ul>';
          foreach ($value['below'] as $k => $sousMenu) {
            $output .= '<li>'.l($sousMenu['link']['link_title'], $sousMenu['link']['link_path']).'</li>';          
          }
          $output .= '</ul>';
        }     
    $output .= '</div>';
